...
=====================================================
v5 change: adds a sector-level heatmap (daily / 1-week / 1-month / 3-month
performance) computed across the FULL NSE 500 universe, plus a weighted
composite score highlighting the top 3 sectors overall.
"""
...
# Weights for the sector composite score (longer horizons weighted more,
# since they reflect a more durable trend rather than daily noise).
...
def get_nse500_universe():
    """Returns a dict {symbol: (full_company_name, sector)}."""
...
# --------------------------------------------------------------------------
# 5. Build dataset (now includes Return3M for the sector heatmap)
# --------------------------------------------------------------------------
...
universe_map = get_nse500_universe()      # {symbol: (company name, sector)}
...
if data.empty:
    ...
    if not sector_perf.empty:
        ...
        st.caption(
            ...
            "10% x daily, averaged across all NSE 500 stocks in each sector. Longer "
            "horizons are weighted more heavily so a single hot day doesn't distort the ranking."
        )
    else:
