...
=====================================================
v3 changes:
  - DailyRet > 0 guard added (guarantees monthly/weekly/daily all positive
    since ordering already forces monthly > weekly > daily).
  - Thresholds loosened so more stocks qualify.
  - MFI trend check: today's MFI must be RISING vs the avg MFI of the
    prior 10 trading days (~2 weeks).
  - Analyst consensus ("Buy Score out of 5") pulled via yfinance for each
    shortlisted stock (only, to keep it fast).
  - Best-effort per-stock FII holding-change scrape (quarterly data is all
    that exists publicly) + a screener.in link per stock as a reliable
    manual fallback, since NSE doesn't expose a clean rolling FII % API.
...
PRICE_CAP = 1500
RSI_MIN = 55                # loosened from 60
ADX_MIN = 20                # loosened from 25
RVOL_MIN = 1.2               # loosened from 1.5
MFI_MIN, MFI_MAX = 45, 90    # loosened from 50-90
LOW_LOOKBACK = 180
LOW_RECENCY_MAX_DAYS = 150   # loosened from 120
PCT_ABOVE_LOW_MIN = 5        # loosened from 8
PCT_ABOVE_LOW_MAX = 80       # loosened from 60
MFI_TREND_LOOKBACK = 10      # ~2 trading weeks
...
def get_fii_holding_change(symbol):
    """
    Best-effort: NSE only discloses FII/FPI holding % quarterly via
    shareholding-pattern filings, not a rolling daily/6-month feed.
    Returns (pct_point_change, screener_link) — pct_point_change may be None
    if the scrape fails, in which case use the link to check manually.
    """
    ...
    try:
        ...
        if r.status_code == 200:
            js = r.json()
            # Structure varies / often only contains filing links, not numeric %.
            # Left as a hook: if NSE ever returns numeric FII% by period, parse here.
            return None, link
    except Exception:
        pass
    return None, link
...
def get_analyst_rating(symbol):
    ...
    We flip it to Buy Score /5 where 5 = strongest buy consensus.
    """
    try:
        ...
        buy_score = round(6 - rec_mean, 1)  # flip scale
...
def build_screener_dataset(symbols, period="9mo"):
    ...
    for i in range(0, len(tickers), chunk):
        ...
        for t in batch:
            ...
            # MFI trend: today's MFI vs avg MFI of the prior 10 sessions
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Screening thresholds are fixed in code but loosened vs earlier version "
               "to surface more candidates. Analyst ratings via yfinance consensus. "
...
if data.empty:
    ...
    if filtered.empty:
        ...
        # Enrich only the shortlisted rows with analyst + FII lookups (keeps it fast)
        ...
        display_cols = ["Symbol", "Price", "RSI", "ADX", "MFI", "MFI_2wAvg", "RVOL",
