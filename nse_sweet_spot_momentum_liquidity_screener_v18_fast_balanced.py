"""
NSE Fast Balanced Momentum + Liquidity Screener
===============================================
v18: Keeps the user's required recent return pattern while deliberately loosening
secondary filters so the screener returns a broader, still-tradeable candidate set.
It also uses a two-stage data pipeline for materially faster Streamlit loading.

PRICE RANGE (unchanged):
  - Rs 99 to Rs 1200

CORE RETURN PATTERN (hard rule):
  - 1-day return > 0
  - 1-week return > 1-day return
  - 1-month return > 1-week return
  - 3-month and 6-month returns must be positive
  - 3M > 1M and 6M > 3M are rewarded in scoring, not required by default
  - Optional Perfect Ladder mode still requires 1D < 1W < 1M < 3M < 6M

BALANCED LIQUIDITY (default):
  - Average 20-day volume >= 30,000 shares
  - Average 20-day traded value >= Rs 0.75 crore
  - Median 20-day traded value >= Rs 0.40 crore
  - At least 90% of recent sessions active
  - Recent turnover cannot have collapsed below 35% of its 20-day average
  - Turnover stability and very-high-turnover-day counts are scoring inputs, not hard gates

NOT OVERBOUGHT:
  - RSI stays below 70 and MFI below 80
  - Extension, Stoch RSI and breakout-spike checks remain, but with wider safety bands
  - High-quality setups are ranked higher when they sit in the RSI/MFI sweet spot

FAST STREAMLIT PIPELINE:
  - Stage 1 downloads only 6 months for the full NSE universe and applies price,
    liquidity, and the exact 1D<1W<1M pattern first
  - Stage 2 downloads 1 year only for those shortlisted symbols and computes the
    heavier technical indicators
  - Larger Yahoo Finance batches plus Streamlit caching reduce repeated load time

RANKING:
  - CompositeScore remains 0-100 and rewards smooth uptrends, longer-horizon ladder
    quality, relative strength, liquidity, accumulation and controlled extension
"""
...
TQ_MFI_MIN = 40
TQ_MFI_MAX = 79.5
TQ_STOCH_RSI_HARD_MAX = 97
TQ_MAX_DAILY_RETURN = 6.0
TQ_MAX_DISTANCE_FROM_52W_HIGH = -35.0
TQ_MAX_EXTENSION_ATR = 3.5
TQ_MAX_PRICE_ABOVE_EMA20_PCT = 12.0
TQ_MAX_BREAKOUT_EXTENSION_PCT = 7.5
TQ_MAX_OVERBOUGHT_RISK = 70.0
TQ_MIN_CONFIRMATION_PCT = 48.0
TQ_MIN_POSITIVE_WEEKS_8 = 37.5
TQ_MIN_TREND_CONSISTENCY_20 = 50.0
TQ_MIN_TREND_R2_60 = 0.02
...
LIQUIDITY_PRESETS = {
    "Balanced (recommended)": {
        ...
        "min_avg_traded_value": 7_500_000,        # Rs 0.75 crore/day
        "min_median_traded_value": 4_000_000,     # Rs 0.40 crore/day
        "min_active_days_pct": 90.0,
        "min_recent_turnover_ratio": 0.35,
        "description": "Best balance: removes genuinely thin stocks without shrinking the list too much.",
    },
    "Broader liquid": {
        "min_avg_volume": 15_000,
        "min_avg_traded_value": 4_000_000,        # Rs 0.40 crore/day
        "min_median_traded_value": 2_000_000,     # Rs 0.20 crore/day
        "min_active_days_pct": 85.0,
        "min_recent_turnover_ratio": 0.25,
        "description": "More results while still excluding very thin / hard-to-trade names.",
    },
    "High liquidity": {
        ...
        "min_recent_turnover_ratio": 0.50,
        "description": "Stronger turnover for larger positions, with fewer qualifying stocks.",
    },
}
DEFAULT_LIQUIDITY_PRESET = "Balanced (recommended)"
...
# ---------------- Fast first-pass snapshot ----------------
@st.cache_data(ttl=CACHE_TTL_SECONDS, show_spinner=False)
def build_fast_snapshot(symbols: Tuple[str, ...], period: str = FAST_PERIOD) -> pd.DataFrame:
    """Cheap full-universe pass: only returns + practical liquidity metrics.

    This intentionally avoids RSI/ADX/MACD/regression/SMA200 calculations for the
    entire NSE universe. Those heavier calculations are done only after the fast
    shortlist is created.
    """
    tickers = [f"{symbol}.NS" for symbol in symbols]
    rows = []

    for start_index in range(0, len(tickers), FAST_CHUNK_SIZE):
        batch = tickers[start_index:start_index + FAST_CHUNK_SIZE]
        try:
            raw = yf.download(
                batch,
                period=period,
        ...
        for ticker in batch:
            ...
            frame = _extract_ticker_frame(raw, ticker, single_ticker_batch=single)
            ...
            median_turnover_20 = turnover.tail(20).median()

            rows.append({
                "Symbol": symbol,
                "Price": safe_round(last_close, 2),
                "DailyRet": safe_round(period_return(close, 1), 2),
                "WeeklyRet": safe_round(period_return(close, 5), 2),
                "MonthlyRet": safe_round(period_return(close, 21), 2),
                "Return3M": safe_round(period_return(close, 63), 2),
                "AvgVol20": int(avg_volume_20) if pd.notna(avg_volume_20) else 0,
                "AvgTradedValue": safe_round(avg_turnover_20, 0),
                "MedianTradedValue20": safe_round(median_turnover_20, 0),
                "ActiveDays20Pct": safe_round((volume.tail(20) > 0).mean() * 100, 1),
                "RecentTurnoverRatio": safe_round(
                    turnover.tail(5).mean() / avg_turnover_20
                    if pd.notna(avg_turnover_20) and avg_turnover_20 > 0 else np.nan,
                    2,
                ),
            })
...
def passes_fast_prefilter(row: pd.Series, preset_name: str) -> bool:
    """Fast hard rules only: price, usable liquidity, and the user's core ladder."""
    config = LIQUIDITY_PRESETS.get(preset_name, LIQUIDITY_PRESETS[DEFAULT_LIQUIDITY_PRESET])
    required = [
        "Price", "DailyRet", "WeeklyRet", "MonthlyRet", "Return3M",
        "AvgVol20", "AvgTradedValue", "MedianTradedValue20",
        "ActiveDays20Pct", "RecentTurnoverRatio",
    ]
    ...
    if row["Return3M"] <= 0:
        return False
    if row["DailyRet"] > TQ_MAX_DAILY_RETURN:
        return False
...
@st.cache_data(ttl=CACHE_TTL_SECONDS, show_spinner=False)
def build_dataset(symbols: Tuple[str, ...], period: str = DATA_PERIOD) -> pd.DataFrame:
...
def liquidity_grade(row: pd.Series) -> str:
    ...
    if pd.isna(turnover) or pd.isna(stability):
        return "Unavailable"
    if turnover >= 250_000_000 and stability >= 0.40:
        return "Excellent"
    if turnover >= 100_000_000 and stability >= 0.30:
        return "High"
    if turnover >= 7_500_000 and stability >= 0.20:
        return "Good"
...
def passes_liquidity_gate(
    ...
    """Practical liquidity floor without over-filtering.

    Share volume + rupee turnover stay hard requirements. Stability, RVOL and
    high-turnover-day counts remain ranking inputs instead of extra hard gates.
    """
    config = LIQUIDITY_PRESETS.get(preset_name, LIQUIDITY_PRESETS[DEFAULT_LIQUIDITY_PRESET])
    required = [
        "AvgVol20", "AvgTradedValue", "MedianTradedValue20",
    ...
    if any(pd.isna(row.get(column, np.nan)) for column in required):
        return False
    return bool(
        row["AvgVol20"] >= config["min_avg_volume"]
        and row["AvgTradedValue"] >= config["min_avg_traded_value"]
        and row["MedianTradedValue20"] >= config["min_median_traded_value"]
        and row["ActiveDays20Pct"] >= config["min_active_days_pct"]
        and row["RecentTurnoverRatio"] >= config["min_recent_turnover_ratio"]
    )


def passes_anti_overbought_gate(row: pd.Series) -> bool:
    """Keep true overbought/chasing risk out, without rejecting healthy momentum."""
    required = [
    ...
    if any(pd.isna(row.get(column, np.nan)) for column in required):
        return False
    if not (TQ_RSI_MIN <= row["RSI"] < RSI_OVERBOUGHT):
        return False
    if not (TQ_MFI_MIN <= row["MFI"] < MFI_OVERBOUGHT):
        return False
...
def passes_return_staircase(
    ...
    """Exact recent ladder; long-horizon ladder is soft unless Perfect mode is on."""
    required = ["DailyRet", "WeeklyRet", "MonthlyRet", "Return3M", "Return6M"]
    if any(pd.isna(row.get(column, np.nan)) for column in required):
        return False

    # This is the user's non-negotiable chart shape.
    if not (0 < row["DailyRet"] < row["WeeklyRet"] < row["MonthlyRet"]):
        return False
    ...
    # Default balanced mode deliberately does NOT hard-require 1M<3M<6M.
    # Those two steps are strongly rewarded by ReturnStaircaseScore instead.
    return True
...
def passes_technical_quality(
    ...
    """Essential uptrend gate; scoring does the finer quality discrimination.

    v17 made too many secondary indicators mandatory at once.  v18 keeps only
    the conditions needed to stop clearly weak/downtrend setups from entering.
    The CompositeScore then ranks the best technical structures to the top.
    """
    required = [
        "HistoryDays", "Price", "EMA20", "EMA50", "EMA20Slope10", "EMA50Slope20",
        "ADX", "PlusDI", "MinusDI", "RSI", "MFI", "StochRSI", "MACD_hist_pct",
        "MACDAccel", "ROC10", "DailyRet", "WeeklyRet", "MonthlyRet", "Return3M",
        "Return6M", "Breakout20Pct", "ExtensionATR", "PriceAboveEMA20Pct",
        "TrendConsistency20", "MaxDrawdown63", "ConfirmationPct", "OverboughtRisk",
    ]
    if any(pd.isna(row.get(column, np.nan)) for column in required):
        return False
    ...
    # Essential chart direction: above a rising short trend and not materially
    # below the medium trend.  EMA20>EMA50 is preferred in scoring, not mandatory.
    if row["Price"] <= row["EMA20"]:
        return False
    if row["Price"] < row["EMA50"] * 0.98:
        return False
    if row["EMA20Slope10"] <= 0:
        return False
    if row["EMA50Slope20"] < -1.0:
        return False
    ...
    # Need at least one trend-strength confirmation instead of demanding all of them.
    if row["ADX"] < 15 and row["TrendConsistency20"] < 55:
        return False
    if row["PlusDI"] <= row["MinusDI"] and row["MACD_hist_pct"] <= 0 and row["MACDAccel"] <= 0:
        return False

    # Reject only clearly deteriorating intermediate momentum.
    if row["ROC10"] < -2.0:
        return False

    sma200 = row.get("SMA200", np.nan)
    if pd.notna(sma200) and row["Price"] < sma200 * 0.92:
        return False
    sma200_slope = row.get("SMA200Slope20", np.nan)
    if pd.notna(sma200_slope) and sma200_slope < -2.0:
        return False

    if row["MaxDrawdown63"] < -35:
        return False
    pct_from_high = row.get("PctFrom52WHigh", np.nan)
    if pd.notna(pct_from_high) and pct_from_high < -40:
        return False
    if row["ConfirmationPct"] < 40:
        return False
    return True
...
def build_composite_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["LiquidityScore"] = (
        linear_score(log_avg_turnover, np.log10(7_500_000), np.log10(1_000_000_000)) * 0.32
        + linear_score(log_median_turnover, np.log10(4_000_000), np.log10(500_000_000)) * 0.22
        + linear_score(log_avg_volume, np.log10(30_000), np.log10(2_000_000)) * 0.14
        + linear_score(d["ActiveDays20Pct"], 90, 100) * 0.07
        + linear_score(d["TurnoverStability20"], 0.20, 0.85) * 0.10
        + sweet_spot_score(d["RecentTurnoverRatio"], 0.35, 1.05, 2.50) * 0.09
        + linear_score(d["DaysAbove75LakhPct"], 35, 100) * 0.06
    )
...
def build_setup_summary(row: pd.Series) -> str:
    ...
    if bool(row.get("FullReturnStaircase", False)):
        ...
        tags.append("Strong long-horizon ladder")
    elif bool(row.get("CoreReturnStaircase", False)):
        tags.append("1D<1W<1M rising ladder")
...
# ---------------- UI ----------------
st.title("NSE Fast Balanced Momentum & Liquidity Screener")
st.caption(
    "v18 uses a fast 2-stage scan: the full NSE universe gets a lightweight 6-month pass first; "
    "only the shortlisted names receive the heavier 1-year technical analysis."
)

if HAS_AUTOREFRESH:
    # Reruns hourly, but cached market data is reused for 30 minutes.
...
with st.sidebar:
    ...
    liquidity_preset = st.selectbox(
        ...
        help=(
            "Balanced is the default sweet spot. Use Broader liquid if you want more names; "
            "High liquidity is stricter for larger positions."
        ),
    )
    require_perfect_staircase = st.checkbox(
        ...
        value=False,
        help=(
            "Off by default. Balanced mode keeps your exact 1D<1W<1M pattern and positive "
            "3M/6M returns, but treats the 3M>1M and 6M>3M steps as scoring bonuses."
        ),
    )
    refresh_btn = st.button("Refresh market data now")
...
st.info(
    f"**{liquidity_preset}:** {liquidity_config['description']}  "
    f"Minimum avg volume **{liquidity_config['min_avg_volume']:,}** shares/day; "
    f"avg turnover **Rs {liquidity_config['min_avg_traded_value'] / 10_000_000:.2f} Cr/day**."
)
st.caption(
    f"Hard pattern: **0 < 1D < 1W < 1M**, with positive 3M and 6M returns. "
    f"RSI stays below {RSI_OVERBOUGHT} and MFI below {MFI_OVERBOUGHT}. "
    "3M>1M and 6M>3M improve the score but are not hard requirements unless Perfect ladder is enabled. "
    "Turnover is a practical liquidity proxy; daily Yahoo data does not provide live bid-ask spread/order-book depth."
)

universe_map = get_full_nse_universe()
sector_map = get_sector_overlay()
symbols = tuple(universe_map.keys())

# -------- Stage 1: lightweight full-universe pass --------
with st.spinner(f"Fast pass: screening {len(symbols):,} NSE stocks for price, liquidity and return pattern..."):
    fast_data = build_fast_snapshot(symbols)

if fast_data.empty:
    st.error("Could not fetch the fast market snapshot. Try 'Refresh market data now'.")
else:
    fast_data["Company Name"] = fast_data["Symbol"].map(lambda s: universe_map.get(s, s))
    fast_data["Sector"] = fast_data["Symbol"].map(lambda s: sector_map.get(s, "Unclassified"))

    fast_shortlist = fast_data[
        fast_data.apply(lambda row: passes_fast_prefilter(row, liquidity_preset), axis=1)
    ].copy()

    st.write(
        f"**Fast scan:** {len(fast_data):,} symbols with usable data -> "
        f"**{len(fast_shortlist):,}** passed price + liquidity + exact 0<1D<1W<1M + positive 3M."
    )

    if fast_shortlist.empty:
        st.info(
            "No names passed the exact recent return ladder in this snapshot. "
            "Try 'Broader liquid' if liquidity is the bottleneck; the return ladder itself remains unchanged."
        )
        data = pd.DataFrame()
    else:
        candidate_symbols = tuple(fast_shortlist["Symbol"].tolist())

        # -------- Stage 2: heavy indicators only on shortlisted names --------
        with st.spinner(
            f"Deep technical pass: analyzing only {len(candidate_symbols):,} shortlisted stocks..."
        ):
            data = build_dataset(candidate_symbols)
    ...
    if not data.empty:
        ...
        benchmark_used = (
            data["Benchmark"].dropna().iloc[0]
            if data["Benchmark"].notna().any()
            else "Unavailable"
        )

        st.subheader(f"Top {top_n} Balanced Rising-Momentum Stocks")
        ...
        not_overbought_ok = liquid_ok[liquid_ok.apply(passes_anti_overbought_gate, axis=1)].copy()
        staircase_ok = not_overbought_ok[
            not_overbought_ok.apply(
                lambda row: passes_return_staircase(row, require_perfect_staircase), axis=1
            )
        ].copy()
        technical_ok = staircase_ok[
            staircase_ok.apply(
                lambda row: passes_technical_quality(row, require_perfect_staircase), axis=1
            )
        ].copy()
        ...
        staircase_label = (
            ...
            if require_perfect_staircase
            else "balanced 1D<1W<1M + positive 3M/6M"
        )
        st.write(
            f"**Deep pass:** {len(data)} shortlisted names analyzed -> "
            f"{len(basic_ok)} passed outer price/overbought limits -> "
            f"{len(liquid_ok)} passed liquidity -> "
            f"{len(not_overbought_ok)} were not stretched -> "
            f"{len(staircase_ok)} passed **{staircase_label}** -> "
            f"**{len(technical_ok)} final candidates**. Benchmark: **{benchmark_used}**."
        )

        if technical_ok.empty:
            st.info(
                "No names cleared every secondary technical check today. The core return pattern is already balanced; "
                "switching to 'Broader liquid' is the least aggressive way to increase candidates without changing it."
            )
        else:
            scored = build_composite_scores(technical_ok)
            result = scored.sort_values(
                ["CompositeScore", "MomentumScore", "TrendScore", "LiquidityScore", "AvgTradedValue"],
            ...
            with st.expander("Verify the return ladder, liquidity and overbought risk"):
                audit_df = result[[
                    "Company Name", "Price", "DailyRet", "WeeklyRet", "MonthlyRet", "Return3M",
                    "Return6M", "LongLadderStrictSteps", "AvgVol20", "AvgTradedValueCr",
                    "MedianTradedValueCr", "LiquidityGrade", "RSI", "MFI", "StochRSI",
                    "OverboughtRisk", "ExtensionATR", "PriceAboveEMA20Pct", "CompositeScore",
                    "SetupSummary",
                ]].copy()
                audit_df = audit_df.rename(columns={
                    "DailyRet": "1D %", "WeeklyRet": "1W %", "MonthlyRet": "1M %",
                    "Return3M": "3M %", "Return6M": "6M %",
                    "LongLadderStrictSteps": "Long Ladder Steps Passed",
                    "AvgVol20": "Avg Volume (20D)", "AvgTradedValueCr": "Avg Turnover Rs Cr",
                    "MedianTradedValueCr": "Median Turnover Rs Cr", "LiquidityGrade": "Liquidity",
                    "StochRSI": "Stoch RSI", "OverboughtRisk": "Overbought Risk (lower better)",
                    "ExtensionATR": "EMA20 Extension (ATR)", "PriceAboveEMA20Pct": "% Above EMA20",
                    "CompositeScore": "Score", "SetupSummary": "Why it passed",
                }).reset_index(drop=True)
                audit_df.index = audit_df.index + 1
                st.dataframe(audit_df, use_container_width=True)
                st.caption(
                    "Default mode hard-requires only the recent ladder 0<1D<1W<1M plus positive 3M/6M. "
                    "A stock gets extra ranking credit when 3M>1M and 6M>3M, instead of being excluded for a small long-horizon mismatch."
                )
            ...
            st.download_button(
                ...
                file_name="nse_fast_balanced_momentum_liquidity_v18.csv",
            )

    # Sector heatmap comes from the FAST snapshot, so it stays broad and avoids
    # forcing a 1-year download for every NSE symbol.
    st.markdown("---")
    st.subheader("Sector Performance Heatmap (Nifty 500 overlay only)")
    classified = fast_data[fast_data["Sector"] != "Unclassified"]
    grouped = classified.groupby("Sector")[["DailyRet", "WeeklyRet", "MonthlyRet", "Return3M"]].mean()
    ...
    sector_performance = grouped.round(2).sort_values("CompositeScore", ascending=False).reset_index()

    if not sector_performance.empty:
        heat_df = sector_performance.set_index("Sector")[["DailyRet", "WeeklyRet", "MonthlyRet", "Return3M"]]
    ...
    st.caption(
        f"Last app run: {dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
        "market-data cache: 30 minutes | auto-rerun: hourly"
    )
