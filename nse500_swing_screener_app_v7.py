...
=====================================================
v7 fixes:
  - BUG FIX: Price < Rs 1500 cap was never applied to the "Recovery Leaders"
    section (only the main shortlist checked it). Now enforced everywhere.
  - Recovery Leaders logic tightened to genuinely mean "SUPER STRONG
    momentum regained after a significant fall" — not just any small
    positive tick in MFI/ADX. Now requires RSI>=60, MACD bullish, ADX>=25,
    MFI/ADX accelerating by at least 5 points in 10 sessions, and real
    volume (RVOL>=1.3) — tuned for swing traders who want conviction moves,
    not noise.
"""
...
PRICE_CAP = 1500          # HARD RULE: every section must respect this
RSI_MIN = 55
ADX_MIN = 20
RVOL_MIN = 1.2
...
# --- Recovery Leaders: "super strong momentum AFTER a significant fall" ---
RECOVERY_MIN_DRAWDOWN = 12      # must have fallen at least 12% from its high
RECOVERY_MIN_BOUNCE = 3         # must have bounced at least 3% off the low
RECOVERY_MIN_RVOL = 1.3         # real volume conviction
RECOVERY_RSI_MIN = 60           # SUPER strong momentum (stricter than main list's 55)
RECOVERY_ADX_MIN = 25           # SUPER strong trend (stricter than main list's 20)
RECOVERY_MIN_MFI_ACCEL = 5      # MFI must have risen at least 5 pts in ROC_LOOKBACK sessions
RECOVERY_MIN_ADX_ACCEL = 5      # ADX must have risen at least 5 pts in ROC_LOOKBACK sessions
...
# --------------------------------------------------------------------------
# 6. Main strict shortlist filter logic
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# 6c. Top 10 Recovery Leaders — FIXED: price cap enforced + stricter
#     "super strong momentum regained" criteria for genuine swing setups.
# --------------------------------------------------------------------------
def build_recovery_leaders(data_with_meta, top_n=10):
    d = data_with_meta.copy()
    mask = (
        (d["Price"] < PRICE_CAP) &                          # <-- FIX: price cap now enforced here too
        (d["MaxDrawdownPct"] >= RECOVERY_MIN_DRAWDOWN) &
        (d["PctAboveLow"] >= RECOVERY_MIN_BOUNCE) &
        (d["RSI"] >= RECOVERY_RSI_MIN) &                     # super strong momentum
        (d["MACD_bullish"] == True) &                        # confirmed bullish crossover
        (d["ADX"] >= RECOVERY_ADX_MIN) &                      # super strong trend
        (d["MFI_Change_10d"] >= RECOVERY_MIN_MFI_ACCEL) &     # MFI accelerating meaningfully
        (d["ADX_Change_10d"] >= RECOVERY_MIN_ADX_ACCEL) &     # ADX accelerating meaningfully
        (d["RVOL"] >= RECOVERY_MIN_RVOL)                      # real volume conviction
    )
    cand = d[mask].copy()
    if cand.empty:
        return cand
    ...
    cand["RecoveryScore"] = (
        cand["MaxDrawdownPct"] * 0.15 +
        cand["PctAboveLow"] * 0.20 +
        cand["RSI"] * 0.15 +
        cand["ADX"] * 0.15 +
        cand["MFI_Change_10d"] * 0.15 +
        cand["ADX_Change_10d"] * 0.10 +
        cand["RVOL"].clip(upper=5) * 4 * 0.10
    )
    cand["Why"] = cand.apply(
        lambda r: (
            f"Fell {r['MaxDrawdownPct']}% from its high, then bounced {r['PctAboveLow']}% off the low "
            f"({r['DaysSinceLow']} sessions ago); momentum has come back SUPER strong: RSI {r['RSI']}, "
            f"MACD bullish, ADX {r['ADX']} (trend strength). MFI up {r['MFI_Change_10d']} pts and ADX up "
            f"{r['ADX_Change_10d']} pts in the last {ROC_LOOKBACK} sessions (rapid acceleration), on "
            f"{r['RVOL']}x average volume (real conviction, not a fluke bounce). Price Rs {r['Price']} "
            f"(under the Rs {PRICE_CAP} cap)."
        ),
        axis=1,
    )
...
st.caption(
    f"All sections respect: price < Rs {PRICE_CAP}. Main shortlist: momentum + trend "
    "confirmed, MFI rising vs 2-week avg, recovering off a low, RISING on all three "
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption(f"Hard rule for ALL sections: price < Rs {PRICE_CAP}. Company names & "
               "sectors sourced from the NSE 500 index CSV. Analyst ratings via yfinance "
               "consensus. FII per-stock change is best-effort — use the screener.in link if blank.")
...
if data.empty:
    ...
    st.subheader(f"Top 10 Recovery Leaders — Super Strong Momentum After a Big Fall (Price < Rs {PRICE_CAP})")
    st.caption(
        "Stocks that fell sharply from a recent high, then came back with genuinely "
        "SUPER STRONG momentum: RSI >= 60, confirmed MACD bullish crossover, ADX >= 25 "
        "(strong trend), with MFI and ADX both accelerating by 5+ points in the last "
        "10 sessions, backed by real volume (1.3x+ average). This is the cleanest signal "
        "for a swing trader looking for a fresh, high-conviction turnaround."
    )
    recovery = build_recovery_leaders(data, top_n=10)
    if recovery.empty:
        st.info("No stocks currently meet the strict recovery-leader criteria. Try the next hourly refresh.")
    else:
        rec_cols = ["Symbol", "Company Name", "Sector", "Price", "MaxDrawdownPct", "PctAboveLow",
                    "DaysSinceLow", "RSI", "ADX", "ADX_Change_10d", "MFI", "MFI_Change_10d", "RVOL",
                    "RecoveryScore", "Why"]
    ...
    # ---------------- Main Strict Shortlist (price cap ALSO enforced here) ----------------
    ...
    if filtered.empty:
        ...
        st.subheader(f"Main Shortlist ({len(result)}) — Price < Rs {PRICE_CAP}")
