"""
NSE 500 Momentum Screener (Soft Composite Score) + Sector Heatmap
====================================================================
v11: Switched from many hard pass/fail filters to a SOFT combined score.
Only two non-negotiable hard gates remain:
  1. Price < Rs 1000
  2. NOT overbought: RSI < 70 AND MFI < 80

Everything else (ADX, MFI strength, RVOL, 10-day ROC, MACD histogram) is
combined into a single weighted CompositeScore -- no other stock gets
excluded just for being moderate on one indicator. This naturally surfaces
many more candidates from the full NSE 500 scan.

DISPLAY: Company Name, Price, CompositeScore (ranked). Full indicator
detail still computed internally and available via CSV export if needed,
but kept out of the main on-screen table.
"""
...
PRICE_CAP = 1000        # hard gate
RSI_OVERBOUGHT = 70     # hard gate ceiling
MFI_OVERBOUGHT = 80     # hard gate ceiling
ROC_LOOKBACK = 10

# Weights for the composite score (soft factors -- sum to 1.0)
...
def get_nse500_universe():
    """Pulls the FULL live NSE 500 constituent list. This is the entire
    universe that gets scanned every refresh -- not a subset."""
...
def build_dataset(symbols, period="6mo"):
    ...
    for i in range(0, len(tickers), chunk):
        ...
        for t in batch:
            ...
            # normalize MACD histogram as % of price so it's comparable across stocks
...
def passes_hard_gates(row):
    """Only 2 non-negotiable rules: price cap + not overbought."""
...
def normalize(series):
    """Min-max normalize a series to 0-100, ignoring NaNs."""
    s = series.copy()
    valid = s.dropna()
...
def build_composite_scores(df):
    ...
    d["CompositeScore"] = round(
        d["N_ADX"] * WEIGHTS["ADX"] +
        d["N_MFI"] * WEIGHTS["MFI"] +
        d["N_RVOL"] * WEIGHTS["RVOL"] +
        d["N_ROC10"] * WEIGHTS["ROC10"] +
        d["N_MACD"] * WEIGHTS["MACD_hist"],
        1
    )
    return d
...
st.caption(
    f"Scans the FULL NSE 500 universe every refresh. Hard rules: price < Rs {PRICE_CAP}, "
    f"NOT overbought (RSI < {RSI_OVERBOUGHT}, MFI < {MFI_OVERBOUGHT}). Everything else "
    "(ADX, MFI strength, volume, momentum, MACD) is blended into one Composite Score -- "
    "no stock is excluded just for being moderate on a single indicator."
)
...
with st.sidebar:
    st.header("Controls")
    top_n = st.slider("How many stocks to show", 10, 100, 30)
...
st.write(f"Scanning **{len(symbols)}** NSE 500 symbols this refresh.")
...
if data.empty:
    ...
    st.write(f"Successfully pulled data for **{len(data)}** of {len(symbols)} symbols "
             "(some may be skipped due to insufficient trading history or temporary data gaps).")
    ...
    if candidates.empty:
        ...
        st.download_button("Download as CSV",
                            result[["Company Name", "Price", "CompositeScore", "RSI", "ADX", "MFI",
                                    "RVOL", "ROC10", "MACD_hist_pct"]].to_csv(index=False).encode(),
                            file_name="nse500_momentum_composite.csv")
