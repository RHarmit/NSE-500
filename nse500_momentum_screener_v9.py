...
==================================================================
v9: Loosened filtering logic so more NSE 500 stocks realistically qualify,
while still enforcing the core ask -- strong momentum, NOT overbought,
price < Rs 1500. Simplified/lighter code: fewer mandatory hard filters,
MFI/volume acceleration now BOOST the ranking score instead of being
pass/fail gates (which was excluding almost everything).

CORE FILTER (hard gates, kept minimal on purpose):
  - Price < Rs 1500
  - RSI between 50-68        (momentum present, NOT overbought >70)
  - MFI below 80              (NOT overbought, no strict floor)
  - ADX >= 20                 (genuinely trending, not choppy)
  - RVOL >= 1.1                (at least slightly above-average volume)

RANKING (score, not a filter -- rewards but doesn't require):
  - ADX rising over the last 10 sessions
  - MFI rising over the last 10 sessions
  - Higher RVOL
  - Higher RSI (within the non-overbought band)

Shows top 25 by default (adjustable in sidebar) instead of a hard-coded 10,
so the list isn't artificially starved.
"""
...
# Hard gates -- kept few and loose so the list isn't near-empty
RSI_MIN, RSI_MAX = 50, 68     # momentum present, but not overbought (70+)
MFI_MAX = 80                  # not overbought (no mandatory floor)
ADX_MIN = 20                  # genuinely trending
RVOL_MIN = 1.1                # at least mild above-average volume
ROC_LOOKBACK = 10             # ~2 weeks, used only for scoring boosts
...
def build_dataset(symbols, period="6mo"):
    ...
    for i in range(0, len(tickers), chunk):
        ...
        for t in batch:
            ...
            mfi_change_10d = mfi14.iloc[-1] - mfi14.iloc[-(ROC_LOOKBACK + 1)] if len(mfi14) > ROC_LOOKBACK else 0
            adx_change_10d = adx14.iloc[-1] - adx14.iloc[-(ROC_LOOKBACK + 1)] if len(adx14) > ROC_LOOKBACK else 0
            ...
            rows.append({
                ...
                "ADX_Change_10d": round(adx_change_10d, 1) if not pd.isna(adx_change_10d) else 0,
                "MFI": round(mfi14.iloc[-1], 1) if not pd.isna(mfi14.iloc[-1]) else np.nan,
                "MFI_Change_10d": round(mfi_change_10d, 1) if not pd.isna(mfi_change_10d) else 0,
...
def passes_momentum_rules(row):
    ...
    if pd.isna(row["RSI"]) or not (RSI_MIN <= row["RSI"] <= RSI_MAX):
        return False
    if pd.isna(row["MFI"]) or row["MFI"] > MFI_MAX:
        return False
...
def momentum_score(row):
    return round(
        (row["RSI"] - RSI_MIN) * 0.8 +
        (row["ADX"] - ADX_MIN) * 0.8 +
        max(row["ADX_Change_10d"], 0) * 1.2 +
        max(row["MFI_Change_10d"], 0) * 1.2 +
        min(row["RVOL"], 5) * 3,
        2
    )
...
def explain(row):
    accel_bits = []
    if row["ADX_Change_10d"] > 0:
        accel_bits.append(f"ADX up {row['ADX_Change_10d']} pts/10d")
    if row["MFI_Change_10d"] > 0:
        accel_bits.append(f"MFI up {row['MFI_Change_10d']} pts/10d")
    accel_txt = "; ".join(accel_bits) if accel_bits else "momentum steady (no sharp acceleration yet)"
    return (
        f"RSI {row['RSI']} and MFI {row['MFI']} -- strong but NOT overbought "
        f"(RSI<{RSI_MAX}, MFI<{MFI_MAX}); ADX {row['ADX']} confirms a real trend; "
        f"{accel_txt}; volume {row['RVOL']}x average; price Rs {row['Price']} (under Rs {PRICE_CAP})."
    )
...
def build_sector_performance(data_with_sector):
    ...
    return grp.round(2).sort_values("CompositeScore", ascending=False).reset_index()

# ---------------- UI ----------------
st.title("NSE 500 Momentum Screener")
st.caption(
    f"Stocks under Rs {PRICE_CAP} with strong momentum (RSI {RSI_MIN}-{RSI_MAX}, "
    f"MFI < {MFI_MAX}, ADX >= {ADX_MIN}, volume >= {RVOL_MIN}x average) -- momentum "
    "present and NOT overbought. Ranked by strength, so stronger/accelerating setups "
    "surface first even though acceleration itself isn't a hard requirement."
)
...
with st.sidebar:
    st.header("Controls")
    top_n = st.slider("How many stocks to show", 10, 60, 25)
...
if data.empty:
    ...
    if candidates.empty:
        ...
        st.write(f"{len(candidates)} stocks passed the filter out of {len(data)} scanned; showing top {len(result)}.")
        ...
        st.download_button("Download as CSV",
                            ...
                            file_name="nse500_momentum_stocks.csv")
    ...
    if not sector_perf.empty:
        ...
        st.caption("Composite score = 40% x 3-month + 30% x 1-month + 20% x 1-week + 10% x daily return, "
                   "averaged across all NSE 500 stocks in each sector.")
    else:
